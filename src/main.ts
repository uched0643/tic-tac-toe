...
let store: number
let gameOverModal: HTMLDivElement = document.getElementById('gameOverUser') as HTMLDivElement
let gameOverModalFailed: HTMLDivElement = document.getElementById('gameOverUserFailed') as HTMLDivElement
let gameOverModalDraw: HTMLDivElement = document.getElementById('gameOverDraw') as HTMLDivElement
let isClicked: any[] = []
let userPosition: any[] = []
let winScore: any

document.getElementById('restart')?.addEventListener('click', ()=> location.reload())
document.getElementById('ok-btn')?.addEventListener('click', ()=> location.reload())
document.getElementById('ok-btn-draw')?.addEventListener('click', ()=> location.reload())
document.getElementById('ok-btn-failed')?.addEventListener('click', ()=> location.reload())
let winArray = [
    [1, 2, 3],
    [4, 5, 6],
    [7, 8, 9],
    ...
    board[i].addEventListener('click', e => {
        ...
        if (isClicked.length !== 0 && isClicked[isClicked.length - 1].user === 'player') {
            ...
            if (counter > 2) {
                ...
                x = userPosition[userPosition.length - 1][0].find((a: any) => a.includes(x[1].number))

                x = x.filter((a: any) => a !== isClicked.filter((a) => a.user == "player")[0].number)
                x = x.filter((a: any) => a !== isClicked.filter((a) => a.user == "player")[1].number)
...
let AIPlayer = () => {
    ...
    checkPlayerResult = userPosition.length == 0 ? [] : userPosition[userPosition.length - 2] == undefined ? userPosition[userPosition.length - 1][randomNmberFunction(userPosition.length)][checkPlayerResult as unknown as number] : userPosition[userPosition.length - 2][0].filter((a: any) => a.includes(checkPlayerResult))[0]
    ...
    if (isClicked.filter((a) => a.user === "AI").length > 2) {
        ...
        x = userPosition[userPosition.length - 2][0].find((a: any) => a.includes(x[1].number))

        x = x.filter((a: any) => a !== isClicked.filter((a) => a.user == "AI")[0].number)
        x = x.filter((a: any) => a !== isClicked.filter((a) => a.user == "AI")[1].number)
